...
test('复制点位生成不重复 Tag，并按寄存器占用数递增地址', () => {
  assert.equal(buildDuplicatePointTag('temperature', []), 'temperature_copy');
  assert.equal(buildDuplicatePointTag('temperature', ['temperature_copy']), 'temperature_copy_2');
  assert.equal(
    buildDuplicatePointTag('temperature', ['temperature_copy', 'temperature_copy_3']),
    'temperature_copy_2',
  );
