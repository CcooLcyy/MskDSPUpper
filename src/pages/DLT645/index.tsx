...
import { Alert, Button, Card, Col, Form, Input, InputNumber, message, Modal, Row, Select, Switch } from 'antd';
...
const DLT645: React.FC = () => {
  ...
  const modalOpen = linkModalOpen || pointModalOpen || blockModalOpen;
  ...
  const refreshLinks = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      const list = (await api.dlt645ListLinks()).sort((left, right) => {
  ...
  const handleDeleteLink = useCallback(async (connName: string) => {
    ...
    const link = links.find((item) => item.config?.conn_name === connName);
    if (link?.state === 2) {
      messageApi.warning('请先停止运行中的连接，再删除该连接');
      return;
    }
  ...
  const handleStartLink = useCallback(async () => {
    ...
    try {
      await api.dlt645StartLink(connName);
  ...
  const handleStopLink = useCallback(async () => {
    ...
    try {
      await api.dlt645StopLink(connName);
  ...
  const handlePointSubmit = useCallback(async () => {
    ...
    let values: ReturnType<typeof pointForm.getFieldsValue>;
    try {
  ...
  }, [blocks, editingPointIndex, isPointBool, messageApi, pointForm, pointSubmitting, points, selectedConn, runSelectedLinkStopped]);
  ...
  }, [blocks, messageApi, pointSubmitting, points, selectedConn, runSelectedLinkStopped]);
  ...
  }, [blocks, messageApi, pointSubmitting, selectedConn, runSelectedLinkStopped]);
  ...
  const handleBlockSubmit = useCallback(async () => {
    if (!selectedConn || blockSubmitting) {
      return;
    }
    let values: ReturnType<typeof blockForm.getFieldsValue>;
    try {
      values = await blockForm.validateFields();
    } catch {
      return;
    }
  ...
  }, [blocks, blockForm, blockSubmitting, editingBlockIndex, messageApi, points, selectedConn, runSelectedLinkStopped]);
  ...
  }, [blockSubmitting, blocks, messageApi, points, selectedConn, runSelectedLinkStopped]);
  ...
  const renderLinkModal = (): React.ReactNode => (
    ...
    >
      <Form form={linkForm} layout="vertical" size="small" autoComplete="off">
        <div className="dlt645-form-section">
  ...
  const renderBlockModal = (): React.ReactNode => (
    ...
    >
      <Form form={blockForm} layout="vertical" size="small">
        <div className="dlt645-form-section">
          <div className="dlt645-form-section-title">块信息</div>
          <Row gutter={[16, 0]}>
            <Col xs={24} sm={12} lg={8}>
              <Form.Item
        ...
        <div className="dlt645-form-section">
          <div className="dlt645-form-section-title">子项配置</div>
          <div className="dlt645-block-editor">
            <Form.List name="items">
              {(fields, { add, remove }) => (
                <>
                  {fields.map((field) => (
                    <div className="dlt645-block-item-row" key={field.key}>
                      <div className="dlt645-block-item-heading">
                        <span className="dlt645-block-item-index">子项 {field.name + 1}</span>
                        <Button
                          danger
                          type="text"
                          size="small"
                          icon={<MinusCircleOutlined />}
                          aria-label={`删除子项 ${field.name + 1}`}
                          onClick={() => remove(field.name)}
                        >
                          删除
                        </Button>
                      </div>
                      <Row gutter={[12, 0]} className="dlt645-block-item-main">
                        <Col xs={24} sm={12} lg={8}>
                          <Form.Item
                            label="标签"
                            name={[field.name, 'tag']}
                          ...
                          >
                            <Select
                              ...
                              onChange={(value: number) => {
                                if (value !== 1) {
                                  blockForm.setFieldValue(['items', field.name, 'byte_index'], null);
                                  blockForm.setFieldValue(['items', field.name, 'bit_index'], null);
                                }
                              }}
                            />
                          </Form.Item>
                      ...
                      <Form.Item
                        ...
                        shouldUpdate={(previousValues, currentValues) => (
                          previousValues.items?.[field.name]?.data_type
                          !== currentValues.items?.[field.name]?.data_type
                        )}
                      >
                        {({ getFieldValue }) => {
  ...
  return (
    <div className="protocol-page dlt645-page">
      {contextHolder}

      {refreshError ? (
        <Alert
          className="dlt645-page-alert"
          type="warning"
          showIcon
          message="连接列表刷新失败"
      ...
      {currentView === 'config' ? (
        <ResizableSplit
          ...
          minSize={240}
          maxSize={620}
          storageKey="mskdsp.layout.dlt645.config"
        >
          <ResizableSplit
          ...
          >
            <ProtocolConnectionList
              ...
              getDescription={(item) => {
                ...
                const protocolVariant = PROTOCOL_VARIANT_OPTIONS.find(
                  (option) => option.value === config.protocol_variant,
                )?.label ?? '协议未指定';
                const commMode = COMM_MODE_OPTIONS.find(
                  (option) => option.value === config.comm_mode,
                )?.label ?? '通信方式未指定';
                return `${LINK_STATE_LABELS[item.state] ?? '状态未知'} · ${protocolVariant} · ${commMode} · 表计 ${config.meter_addr || '-'}`;
              }}
              getDeleteTitle={(connName) => `\u786e\u8ba4\u5220\u9664 ${connName}\uff1f`}
            />

            <div className="dlt645-connection-shell">
              <ConnectionConfig
                link={selectedLink}
                selectedConn={selectedConn}
          ...
          <PointTable
            ...
            onCopyPoint={(index) => openCopyPoint(index)}
